...
class ApiClient {
  ...
  async login(phone: string, password: string) {
    // Build a list of auth emails to try for this phone number.
    //
    ...
    //
    // We try the real email FIRST (looked up via the get_auth_email_by_phone
    // RPC), then fall back to the synthesized email. This ensures login
    // works regardless of:
    //   - Whether the RPC migration has been run
    //   - Whether the account was created before or after the email change
    //   - Whether the customer's email in the DB matches the auth email
    const synthesizedEmail = phoneToEmail(phone)
    const emailsToTry: string[] = [synthesizedEmail]

    try {
      const { data: rpcEmail } = await supabase.rpc('get_auth_email_by_phone', { p_phone: phone })
      if (rpcEmail && rpcEmail !== synthesizedEmail) {
        // Try the real email FIRST, then fall back to synthesized
        emailsToTry.unshift(rpcEmail)
      }
    } catch {
      // RPC not available (migration not run yet) — only try synthesized email
    }
    ...
    if (!authData) {
      throw new Error(lastError?.message || 'Invalid phone or password')
    }
    ...
    if (!profile) {
      ...
      const newProfile = {
        id: authData.user.id,
        phone,
        email: authData.user.email || synthesizedEmail,
        name: authData.user.user_metadata?.name || phone,
      ...
      return { user: created, token: authData.session?.access_token || '' }
    }
  ...
  /**
   * Start a game: deduct entry cost immediately, record a game_history
   * row with status='started'. Returns the game_id (needed by finishGame)
   * and the new points balance.
   */
  async startGame(gameType: string): Promise<{ gameId: string; entryCost: number; newPointsBalance: number }> {
    const { data: { user: authUser } } = await supabase.auth.getUser()
    if (!authUser) throw new Error('Not authenticated')
  ...
  /**
   * Finish a game: add winnings to the customer's balance, mark the
   * game_history row as 'finished'. Idempotent — calling twice is safe.
   */
  async finishGame(gameId: string, winnings: number): Promise<{ newPointsBalance: number; alreadyFinished: boolean }> {
    ...
    return {
      newPointsBalance: data.new_points_balance,
      alreadyFinished: data.already_finished ?? false,
    }
  }
