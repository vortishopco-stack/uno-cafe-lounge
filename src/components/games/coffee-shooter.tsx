...
import { DEFAULT_WIN_TIERS, type WinTier } from '@/lib/api'
...
export function CoffeeShooterGame({ onEnd, entryCost, winTiers }: CoffeeShooterGameProps) {
  ...
  // Use admin-configured tiers (sorted desc by minScore), else defaults
  const tiers = useMemo(() => {
    const list = winTiers && winTiers.length > 0 ? winTiers : DEFAULT_WIN_TIERS
    return [...list].sort((a, b) => b.minScore - a.minScore)
  }, [winTiers])
  ...
  const calculateWinnings = () => {
    // Use admin-configured tiers (evaluated top-down: first tier reached wins)
    for (const tier of tiers) {
      if (score >= tier.minScore) return tier.reward
    }
    return 0
  }
