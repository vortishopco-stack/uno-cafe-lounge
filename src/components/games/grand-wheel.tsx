...
import { DEFAULT_WHEEL_SEGMENTS, type WheelSegment } from '@/lib/api'
...
export function GrandWheelGame({ onEnd, entryCost, segments }: GrandWheelGameProps) {
  // Use admin-configured segments if provided, else fall back to defaults
  const SEGMENTS = useMemo(() => {
    const list = segments && segments.length > 0 ? segments : DEFAULT_WHEEL_SEGMENTS
    // Guard: wheel needs at least 2 segments to be playable
    return list.length >= 2 ? list : DEFAULT_WHEEL_SEGMENTS
  }, [segments])
  ...
  // Initial draw (also re-draws when admin-configured segments arrive)
  useEffect(() => {
  ...
  // Highest-value segment, for the "Top prize" hint
  const topPrize = useMemo(
    () => SEGMENTS.reduce((max, s) => (s.value > max ? s.value : max), 0),
    [SEGMENTS]
  )
